...
FIGURES.OnePTouch.Tablet <- function(tablet_dat, config, resources, pid, dofig, figdir){
  ...
  all_figures <- list()
  for(trial in 1:dim(tablet_dat$event)[1]){
    ...
    ovr_fig <- FIGURES.overviewTouch(tablet_dat, config, resources, pid, trial)
    all_figures[[tname]] <- append(all_figures[[tname]],ovr_fig)

    if(dofig){
      ggplot2::ggsave(file.path(figdir, sprintf('touchpath_P-%s_C-%s_T-%s.png',
                                                pid, cond, trial)),
  ...
  if(dofig){
    ...
    for(ct in names(all_figures)){
      ...
      cond <- strsplit(ct_split,'-')[[1]][2]
      trialnum <- strsplit(ct_split,'-')[[2]][2]

      thisfig <- magick::image_read(
        file.path(figdir, sprintf('touchpath_P-%s_C-%s_T-%s.png', pid, cond, trialnum)) )
    ...
    magick::image_write(allplots, path = file.path(figdir, sprintf('touchpath_P-%s.png',pid)))
  }
  return(all_figures)
}
...
FIGURES.overviewTouch <- function(dat, config, resources, pid, trialnum){
  figlist <- list()
  ...
  trialdat <- subset(dat$touch, time >= trial_times[trialnum] & time < trial_times[trialnum+1])
  eventdat <- dat$event[trialnum,]
...
#  first_timepoint <- dat$time[1]
  ...
  if(dofig){
    ggplot2::ggsave(file.path(figdir, sprintf('velocityRelativeDirect_%s.png',trial_label$figlabel)),
  ...
  if(dofig){
    ggplot2::ggsave(file.path(figdir, sprintf('positionRelativeDirect_%s.png',trial_label$figlabel)),
...
FIGURES.TouchRelativeDirectLocation.Tablet <- function(dat, config, dofig, figdir, trial_label) {
  ...
  if(dofig){
    ggplot2::ggsave(file.path(figdir, sprintf('locationRelativeDirect_%s.png',trial_label$figlabel)),
                    plot = f1, width = 8, height = 5)
  }
}
...
#  first_timepoint <- dat$time[1]
  ...
  if(dofig){
    ggplot2::ggsave(file.path(figdir, sprintf('TouchDerivatives_%s.png',trial_label$figlabel)),
                    plot = f1, width = 8, height = 5)
  }
...
#  first_timepoint <- dat$time[1]
  ...
  if(dofig){
    ggplot2::ggsave(file.path(figdir, sprintf('TouchDerivativesGrid_%s.png',trial_label$figlabel)),
