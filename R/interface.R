...
smtdemo <- function(writedir = getwd() ){
  ...
  tablet_raw_data_folder <- system.file('extdata',tabdir,'9999', package = 'smtk')
  ...
  message(sprintf('demo participant 9999 raw tablet data stored in %s', file.path(writedir, tabdir,'9999')))
...
#  dat_trialtouch <- (pTouchDat[['COND-taskA_TRIAL-4']][['TOUCH-0']])
    ...
    trialnum <- strsplit(strsplit(trial,'_')[[1]][2],'-')[[1]][2]
    touchnum <- strsplit(touch,'-')[[1]][2]
