...
for (folder in folders) {
  ...
  file <- list.files(path = folder, pattern = "1_touch\\.csv$", full.names = TRUE)
...
ggplot(data = dfall, aes(x = index, y = delta_time, color = factor(block_number), shape = factor(trial_number))) + geom_hline(yintercept = (1:5)*8.667) + geom_quasirandom() + facet_wrap(.~pid)
ggsave('tablet_deltatime_jb.png', width = 12, height = 12)
...
ggsave('tablet_deltatime_jb_all.png', width = 8, height = 6)
